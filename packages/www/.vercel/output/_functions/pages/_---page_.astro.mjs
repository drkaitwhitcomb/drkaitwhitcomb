import { A as AstroError, U as UnknownContentCollectionError, c as createComponent, R as RenderUndefinedEntryError, u as unescapeHTML, a as renderTemplate, d as renderUniqueStylesheet, e as renderScriptElement, f as createHeadAndContent, r as renderComponent, g as createAstro, m as maybeRenderHead } from '../chunks/astro/server_tdyPPOLk.mjs';
import 'kleur/colors';
import { p as pages, u as usePage, $ as $$ApplicationLayout } from '../chunks/usePage_B3kustt9.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('../chunks/_astro_data-layer-content_CXmLyYAE.mjs');
...
const __vite_import_meta_env__ = {"ASSETS_PREFIX": undefined, "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SITE": "https://drkaitwhitcomb.com", "SSR": true};
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('../chunks/_astro_assets_CzDJCboi.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('../chunks/content-modules_BmF8zXEZ.mjs');
...
const getStaticPaths = async () => {
  const pagePaths = entries.map(([key, value]) => {
...
const $$ = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$;
  const RESERVED = /* @__PURE__ */ new Set(["$resource", "$version"]);
  const entries2 = Object.entries(pages).filter(([k]) => !RESERVED.has(k));
  function normalizeSlug2(value, key) {
    const slug = value?.metadata?.slug ?? value?.metaData?.slug ?? key;
    if (slug === "/" || slug === "") return "/";
    return String(slug).replace(/^\//, "");
  }
  const param = Astro2.params.page || "";
  let layoutTitle = "";
  let masonry;
  let isPost = false;
  ...
  if (param.startsWith("posts/")) {
    ...
    const match = entries2.find(([key, value]) => {
      const slug = normalizeSlug2(value, key);
  ...
  return renderTemplate`${!isPost && renderTemplate`${renderComponent($$result, "ApplicationLayout", $$ApplicationLayout, { "title": layoutTitle, "masonry": masonry })}`}${isPost && renderTemplate`${renderComponent($$result, "ApplicationLayout", $$ApplicationLayout, { "title": layoutTitle }, { "main": async ($$result2) => renderTemplate`${maybeRenderHead()}<div class="prose mx-auto px-4 py-8">${PostContent && renderTemplate`${renderComponent($$result2, "PostContent", PostContent, {})}`}</div>` })}`}`;
